...
def profile():
    ...
    if 'loggedin' in session:
        ...
        return render_template('auth/profile.html', account=account, username=session['username'])
...
def edit_profile(modifier_id, act):
	if act == "add":
		return render_template('auth/edit_profile.html', data="", act="add")
	else:
		data = fetch_one(mysql, "users", "id", modifier_id)
	
		if data:
			return render_template('auth/edit_profile.html', data=data, act=act, username=session['username'])
		else:
...
def verify_email():
    ...
    try:
        if request.method == 'POST':
            ...
            return render_template('auth/verify_email.html', msg=msg)
    except jwt.DecodeError:
        flash("Invalid token!", "danger")
        return render_template('auth/verify_email.html', msg='Invalid token')
    except Exception as e:
        flash(str(e), "danger")
        return render_template('auth/verify_email.html', msg='An error occurred')
...
def reset_with_token(token):
    try:
        ...
        return render_template('auth/reset_with_token.html', token=token, _external=True)
    except:
...
# Function for drug recommendations
def predict_recommendations(condition):
    print(f"Selected Condition: {condition}")

    # Case-insensitive search for conditions
    condition_df = df[df['Condition'].str.contains(condition, case=False, na=False)]

    # Print available conditions
    available_conditions = df['Condition'].unique()
    print(f"Available Conditions: {', '.join(available_conditions)}")

    if not condition_df.empty:
        # Get recommended drugs for the selected condition
        condition_df = condition_df.drop_duplicates(subset='Drug', keep='first').head(5)
        recommended_drugs = condition_df['Drug'].tolist()
